...
{
    public class TrackingModelV2 : TrackingModel
    {
        private float squintLeft;
        private float squintRight;
        ...
        public override void UpdateEyeData(ref UnifiedEyeData data, VerboseData external, bool isViveProEye)
        {
            ...
            {
                squintLeft = 1-external.left.eye_openness;
                squintLeft = (float)(-1 * Math.Pow(squintLeft, 2) + 2 * squintLeft);
                data.Left.Openness = external.left.eye_openness - squintLeft;
            }
            if (external.right.GetValidity(SingleEyeDataValidity.SINGLE_EYE_DATA_EYE_OPENNESS_VALIDITY))
            {
                squintRight = 1-external.right.eye_openness;
                squintRight = (float)(-1 * Math.Pow(squintRight, 2) + 2 * squintRight);
                data.Right.Openness = external.right.eye_openness - squintRight;
            }
        ...
        public override void UpdateEyeExpressions(ref UnifiedExpressionShape[] data, EyeExpression external)
        {
            ...
            data[(int)UnifiedExpressions.EyeSquintLeft].Weight = squintLeft + external.left.eye_squeeze;
            data[(int)UnifiedExpressions.EyeSquintRight].Weight = squintRight + external.right.eye_squeeze;

            // Emulator expressions for Unified Expressions. These are essentially already baked into Legacy eye expressions (SRanipal)
            data[(int)UnifiedExpressions.BrowInnerUpLeft].Weight = external.left.eye_wide * 0.9f;
            data[(int)UnifiedExpressions.BrowOuterUpLeft].Weight = external.left.eye_wide * 0.5f;

            data[(int)UnifiedExpressions.BrowInnerUpRight].Weight = external.right.eye_wide * 0.9f;
            data[(int)UnifiedExpressions.BrowOuterUpRight].Weight = external.right.eye_wide * 0.5f;
        ...
        public override void UpdateMouthExpressions(ref UnifiedExpressionShape[] shapes, PredictionData_v2 external)
        {
            unsafe
            {
                ...
                shapes[(int)UnifiedExpressions.MouthRaiserUpper].Weight = Math.Max(0f, external.blend_shape_weight[(int)LipShape_v2.MouthLowerOverlay] - external.blend_shape_weight[(int)LipShape_v2.MouthUpperInside]);
                ...
                shapes[(int)UnifiedExpressions.CheekSquintLeft].Weight = (float)Math.Pow(external.blend_shape_weight[(int)LipShape_v2.MouthSmileLeft], 1.7f);
                shapes[(int)UnifiedExpressions.CheekSquintRight].Weight = (float)Math.Pow(external.blend_shape_weight[(int)LipShape_v2.MouthSmileRight], 1.7f);

                shapes[(int)UnifiedExpressions.MouthDimpleLeft].Weight = (float)Math.Pow(external.blend_shape_weight[(int)LipShape_v2.MouthSmileLeft], 2f) * .8f;
                shapes[(int)UnifiedExpressions.MouthDimpleRight].Weight = (float)Math.Pow(external.blend_shape_weight[(int)LipShape_v2.MouthSmileRight], 2f) * .8f;

                shapes[(int)UnifiedExpressions.MouthStretchLeft].Weight = 
                    external.blend_shape_weight[(int)LipShape_v2.MouthSadRight] * (external.blend_shape_weight[(int)LipShape_v2.MouthUpperUpLeft] * external.blend_shape_weight[(int)LipShape_v2.MouthLowerDownLeft]);
                shapes[(int)UnifiedExpressions.MouthStretchRight].Weight = 
                    external.blend_shape_weight[(int)LipShape_v2.MouthSadRight] * (external.blend_shape_weight[(int)LipShape_v2.MouthUpperUpRight] * external.blend_shape_weight[(int)LipShape_v2.MouthLowerDownRight]);
